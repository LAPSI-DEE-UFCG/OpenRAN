...
from .Check_assinatura_int import Resposta
...
class XappSdl:
    ...
    def __init__(self, thread:bool = True):
        ...
        self.logger = Logger(name="XappSdl", level=Level.DEBUG) # The name is included in each log entry, Levels: DEBUG < INFO < WARNING < ERROR
    ...
    def _loop(self):
        ...
        while not self._shutdown: # True since custom xApp initialization until stop() is called
            ...
            dados = self._xapp.sdl_get(namespace="xappsdl", key="pacote")
            if dados is None:
                dados = normal
            else:
                ...
                if i == 4 or i == 8:
                    ...
                    dados = normal
            self._xapp.sdl_set(namespace="xappsdl", key="pacote", value=dados)
            ...
            if veri == 43200:
                ...
                if "Falha" in infor:
                    self.logger.warning("A integridade do xapp foi comprometida !!!")
                else:
                    self.logger.info("Tudo ok !!!!")
